...
void interrupt isr(void) {
    if (INTCONbits.PEIE == 1 && PIE1bits.SSP1IE == 1 && PIR1bits.SSP1IF == 1) {     // If I2C int enabled and flagged
        ...
        I2C_bitCount = 1;
        SSPIF = 0;
        I2C_data[0] = SSP1BUF;               // Save address
        ...
        while (!(SSP1STATbits.I2C_STOP)) {   // Repeat until "P" indicates STOP bit. To do: add a time-out error escape
            if (SSPIF){
                SSP1CON1bits.CKP = 0;        // Hold clock until ready again
                SSPIF = 0;
                I2C_data[I2C_bitCount++] = SSPBUF;
                SSP1CON1bits.CKP = 1;        // Release the clock
            }
        }
        I2C_length = I2C_bitCount - 1;      // May need to know the length outside of the ISR
        SSPIF = 0;                          // Clear flags
        SSP1STATbits.BF = 0;
        ...
        Usr1 = !UsrSw;
        I2C_data[1] = 0x01;					// Enable command
        I2C_data[2] = 0x02;					// Two bytes to follow
        I2C_data[3] = 0x00;					// 0x00 = both motors
